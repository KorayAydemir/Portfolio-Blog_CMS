        ...
        .items([
            ...S.documentTypeListItems().filter(
                (listItem: ListItemBuilder) =>
                    !ignoredDocTypes.includes(listItem.getId() ?? '')
            ),
            S.listItem()
                .title('Comments')
                .child(
            ...
            S.listItem()
                ...
                .child(() =>
                    S.documentList()
                        .title('Comments')
                        .filter(
                            `_type == "comment" && post == '${slug}' && approved == ${approvedStatus}`
                        )
                )
        )
    ...
    return [
        S.listItem()
            ...
            .child(
                S.list()
                    .title('Posts')
                    .items(postsThatHaveComments('undefined'))
            ),
        S.listItem()
            .title('Disapproved')
            .child(
                S.list().title('Posts').items(postsThatHaveComments('false'))
            ),
        S.listItem()
            .title('Approved')
            .child(
                S.list().title('Posts').items(postsThatHaveComments('true'))
            ),
    ]
}
